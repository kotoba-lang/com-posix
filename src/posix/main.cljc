(ns posix.main
  "Kotodama WASM entrypoint for the Posix clean-room actor (L4) — Clojure port.
  ...
  facts (`posix.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"posix.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "posix-compat" :tier tier :entities entities} 200])
